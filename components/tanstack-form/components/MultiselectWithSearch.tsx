  ...
  const handleChange = React.useCallback(
    (option: { label: string; value: string }, checked: CheckedState) => {
      if (checked === 'indeterminate' || checked === false) {
        valuesRef.current = valuesRef.current.filter((value) => value !== option.value);
      } else {
        valuesRef.current.push(option.value);
      }
      field.handleChange(valuesRef.current.join(separator));
    },
    [field],
  );
          ...
          filteredOptions.map((option, index) => (
            <li key={index} className="flex items-center gap-2">
              <Checkbox
                ...
                id={`${field.name}-${index}`}
                name={field.name}
                checked={valuesRef.current.includes(option.value)}
